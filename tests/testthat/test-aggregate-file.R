test_that("bt_aggregate: single/multi key, multi fun", {
  set.seed(1); n <- 2e5L
  ...
  r1 <- bt_aggregate(p, by = "k", value = "x", fun = c("sum", "mean", "sd", "min", "max", "n"))
  ...
  r2 <- bt_aggregate(p, by = c("g", "k"), value = c("x", "y"), fun = "mean")
...
test_that("bt_aggregate: predicate pushdown (where) fused into the pass", {
  ...
  r <- bt_aggregate(p, by = "g", value = "x", fun = "mean", where = c("x > 0", "y < 0.5"))
  ...
  rs <- bt_aggregate(p, by = "k", value = "x", fun = c("sum", "n"), where = "g == b")
...
test_that("bt_count counts grouped rows", {
  set.seed(3); n <- 1e5L
  ...
  c2 <- bt_count(p, by = c("g", "k"), where = "k >= 10")

...
group, `semi join`, and edges it on `equi join` and (on the common
`col <op> scalar` shape) `filter`; the one operation it trails on is string
`sort`. Against `dplyr` it is faster across the board, often by an order of
magnitude on the grouped operations.

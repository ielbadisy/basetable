...
# collapse defaults to one thread; give it the same budget as basetable.
if (HAVE_CL) {
  .old_collapse <- collapse::get_collapse("nthreads")
  collapse::set_collapse(nthreads = getOption("basetable.threads", 1L))
}
```
...
**`data.table`**, **`dplyr`** and **`collapse`** using the
...
bench_one <- function(label, exprs) {
  keep <- c(TRUE, HAVE_DT, HAVE_DP, HAVE_CL)[seq_along(exprs)]
  exprs <- exprs[keep]
...
```{r mem-note, echo=FALSE, results='asis', eval=!HAVE_PROFMEM}
...
```{r plot-mem, eval=HAVE_PROFMEM, fig.cap="Memory allocated by each expression, as reported by bench (lower is better)."}
...
group and `semi join`, and is level with it on `equi join` and (on the common
`col <op> scalar` shape) `filter`. Against `dplyr` it is faster across the
board, often by an order of magnitude on the grouped operations.

`collapse` is the fastest engine on most of these operations, including
`filter`, `distinct`, grouped `count` and both joins; `basetable` is ahead of
it on grouped `sd`. String `sort` is the operation where the single-thread gap
is widest: `orderrows()` is a stable parallel radix, ~20x faster than base
`order()`, but `collapse` and `data.table` are faster when limited to one
thread.

The memory panel is where `basetable` separates from all three. A grouped
`aggregate` or `count` in `basetable` reduces inside the C++ engine without
building intermediate columns, so it allocates a fraction of a megabyte where
`data.table` and `dplyr` allocate tens and `collapse` a few.

The advantage over `data.table` widens with size and cardinality:
...
if (HAVE_CL) collapse::set_collapse(nthreads = .old_collapse)
```

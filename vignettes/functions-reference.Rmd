...
`basetable` deliberately does **not** ship dplyr-named verbs (`filter()`,
`select()`, `mutate()`, `arrange()`, `summarise()`, `distinct()`, `slice()`,
`relocate()`, `bind_rows()`, `bind_cols()`), so it can be attached alongside
`dplyr` without shadowing its grammar. One practical consequence of the
base-style naming that remains: `basetable` still reuses a handful of true
base-R names, like `subset()`, `merge()`, `transform()`, `split()`, and
`count()`, that `data.table` also defines. If you attach `data.table` in the
same session, whichever package was attached **last** wins for any shared
name and silently shadows the other: call `basetable::fn()` explicitly (or
use the [`conflicted`](https://conflicted.r-lib.org/) package) if you need
both loaded at once.
...
```{r}
data.table::as.data.table(mtcars)[c(1, 3, 5)]
```

...
`split()` breaks a table into a named list of data.tables, one per group, using
`data.table`'s grouped split machinery internally:
...
`applyby()` applies a function to each group through the same `data.table`

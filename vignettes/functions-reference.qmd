...
`basetable` is for people who already know base R and want `data.table`
objects without learning `data.table`'s `[i, j, by]` syntax or dplyr's tidy
...
`aggregate()`, and friends) to `data.table`-backed results without a rewrite.
...
# Mapping helpers {#sec-functional}

`map()`/`pmap()`/`walk()`/`pwalk()`/`reduce()` are lightweight base-R helpers
for working with plain vectors and lists (not tables), useful for the
occasional loop-replacement inside a larger `basetable` pipeline.

```{r}
map(1:3, function(x) x + 0.5)
pmap(list(a = 1:2, b = 10:11), function(a, b) a + b)
walk(c("a", "b"), function(x) x)
per <- pwalk(list(x = c("a", "b")), function(x) x)
reduce(1:4, `+`)
```

...
returns a `data.table` directly, since it's meant to be fast and composable
with further `data.table` operations.
...
`split()` breaks a table into a named list of data.tables, one per group:

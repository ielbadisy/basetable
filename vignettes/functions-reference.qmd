...
`basetable` is a bridge for people who already know base R and want
`data.table`'s speed without learning `data.table`'s `[i, j, by]` syntax or
dplyr's tidy evaluation. It is aimed specifically at two audiences: **teaching**
table manipulation to people learning base R (without the extra cognitive load
of non-standard evaluation or terse `[i, j, by]` syntax), and **migrating** a
codebase already built on base-R table semantics (`subset()`, `merge()`,
`aggregate()`, and friends) to `data.table`-backed speed without a rewrite.
...
One practical consequence of the base-style naming: `basetable` reuses names
like `subset()`, `merge()`, `filter()`, and `count()` that dplyr and
data.table also define. If you attach `dplyr`/`data.table` in the same
session, whichever package was attached **last** wins for any shared name and
silently shadows the other: call `basetable::fn()` explicitly (or use the
...
`"xcol<op>ycol"` (the same convention `data.table` itself uses for `on=`),
